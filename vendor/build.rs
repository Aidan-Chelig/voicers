...
fn link_opus(is_static: bool, opus_build_dir: impl Display) {
    ...
    let opus_build_dir = PathBuf::from(opus_build_dir.to_string());
    let lib_dir = if opus_build_dir.join("lib").exists() {
        opus_build_dir.join("lib")
    } else if opus_build_dir.join("lib64").exists() {
        opus_build_dir.join("lib64")
    } else {
        opus_build_dir.join("lib")
    };
